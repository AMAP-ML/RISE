...
def save_responses_for_validation(batch, tokenizer, output_path):
    ...
    raw_prompt_ids = batch.non_tensor_batch.get("raw_prompt_ids", [None] * batch_size)
    ...
    sample_ids = batch.non_tensor_batch.get("id", [None] * batch_size)
    if isinstance(sample_ids, np.ndarray):
        sample_ids = sample_ids.tolist()

    orig_row_indices = batch.non_tensor_batch.get("orig_row_index", [None] * batch_size)
    if isinstance(orig_row_indices, np.ndarray):
        orig_row_indices = orig_row_indices.tolist()

    questions = batch.non_tensor_batch.get("question", [None] * batch_size)
    if isinstance(questions, np.ndarray):
        questions = questions.tolist()

    def _normalize_token_ids(ids):
        if ids is None:
            return []
    ...
    prompts = [
        tokenizer.decode(_normalize_token_ids(ids), skip_special_tokens=True)
    ...
    with open(output_path, "a", encoding="utf-8") as f:
        for idx, (orig_idx, sample_id, orig_row_index, question, prompt, response) in enumerate(
            zip(dataset_indices, sample_ids, orig_row_indices, questions, prompts, responses)
        ):
            question_raw = question if question is not None else ""
            question_text = question_raw if question_raw else prompt
            f.write(json.dumps({
...
class RayPPOTrainer:
    ...
    def _load_checkpoint(self) -> None:
        ...
        if self.config.trainer.load_dataloader_state:
            ...
            print("Skip loading dataloader state from checkpoint (trainer.load_dataloader_state=false).")
    ...
    def fit(self):
        ...
        for _ in tqdm(range(self.config.trainer.total_epochs), desc="Epoch", position=0):
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                self.global_step += 1
                print(f"Training step: global_step={self.global_step}")
                ...
                with timer("step", timing_raw):
                    ...
                    # NOTE: second-hop generation removed for solver training path.
                    ...
                    with timer("adv", timing_raw):
                        ...
                        # Unlock the TensorDict before writing token-level scores.
                        if hasattr(batch.batch, "is_locked") and batch.batch.is_locked:
                            batch.batch.unlock_()
                        ...
                        batch, advantage_metrics = compute_advantage(
                            batch,
                    ...
                    if self.config.trainer.save_freq > 0 and self.global_step % self.config.trainer.save_freq == 0:
                        with timer("save_checkpoint", timing_raw):
                            print(f"Saving checkpoint: global_step={self.global_step}")
        ...
        # perform validation after training (only if val_freq > 0)
        if self.val_reward_fn is not None and self.config.trainer.val_freq > 0:
            print(f"Running validation: val_freq={self.config.trainer.val_freq}")
            if (
                val_metrics is None
