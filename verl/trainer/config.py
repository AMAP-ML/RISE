...
class DataConfig:
    ...
    # Val-only prompt controls (keep training behavior unchanged by default)
    val_include_options_in_prompt: bool = False
    val_force_choice_letter_output: bool = False
    val_force_yes_no_output: bool = False
    val_option_keys: Tuple[str, ...] = ("options", "choices")
    # Eval-only image placeholder normalization extension (e.g. <image1>, <image_1>)
...
class TrainerConfig:
    ...
    # For some pipelines (e.g., solver with regenerated parquet each micro),
    # we may want to resume model/optimizer/global_step but not dataloader cursor.
