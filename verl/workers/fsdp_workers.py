...
class FSDPWorker(Worker):
    def __init__(
        ...
        if not dist.is_initialized():
            dist_timeout_sec = int(os.getenv("TORCH_DIST_TIMEOUT_SEC", "3600"))
            ...
            try:
                ...
                dist.init_process_group(backend="nccl", timeout=timedelta(seconds=dist_timeout_sec))
    ...
    def _build_model_optimizer(
        ...
        if fsdp_config.torch_dtype is None:
            torch_dtype = PrecisionType.to_dtype(fsdp_config.mp_param_dtype)
        else:
